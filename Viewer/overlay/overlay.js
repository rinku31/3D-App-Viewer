...
function setupGlobalListeners() {
  ...
  // Close persistent panels when clicking outside any hotspot, panel, or viewer UI
...
export function buildHotspotOverlays() {
  ...
  const globalBtnFontColor = state.sceneDocument?.settings?.hotspots?.btnFontColor || "#ffffff";
  const globalBtnFontSize = Number(state.sceneDocument?.settings?.hotspots?.btnFontSize || 11);
  const globalBtnBgColor = state.sceneDocument?.settings?.hotspots?.btnBgColor || "rgba(68, 214, 44, 0.28)";
  const globalBtnPaddingV = Number(state.sceneDocument?.settings?.hotspots?.btnPaddingV || 5);
  const globalBtnPaddingH = Number(state.sceneDocument?.settings?.hotspots?.btnPaddingH || 12);
  const globalBtnMargin = Number(state.sceneDocument?.settings?.hotspots?.btnMargin || 5);
    ...
    if (paragraphs.length > 0) {
      paragraphs.forEach((para, pIdx) => {
        ...
        if (text) {
          ...
          paraText.style.fontSize = `${globalDescFontSize}px`;
          paraText.style.color = globalDescFontColor;
        ...
        if (enabledButtons.length > 0) {
          ...
          btnGroup.style.gap = `${globalBtnMargin}px`;
          btnGroup.style.marginTop = `${globalBtnMargin}px`;
          ...
          enabledButtons.forEach((btnData) => {
            ...
            btn.style.padding = `${globalBtnPaddingV}px ${globalBtnPaddingH}px`;
            btn.style.fontSize = `${globalBtnFontSize}px`;
            btn.style.fontWeight = "600";
            btn.style.borderRadius = "5px";
            btn.style.border = "1px solid rgba(255,255,255,0.18)";
            btn.style.background = globalBtnBgColor;
            btn.style.color = globalBtnFontColor;
